...
namespace Camvox
{
  ...
  void Calibrating::BuildLidarCloud()
  {
    ...
    convertToRgbCloud(lidar_cloud_xyz_, lidar_cloud_xyzrgb_, Eigen::Vector3i(0, 0, 255)); // blue
  }

  //*************************************************costFunction*********************************************************************/
  // 用于优化激光点云和图像之间外参的代价函数
  ...
  // update parameter 抛弃异常值（超过上限，或超过下限），根据均值可信范围，选择是否接受标定的结果并更新到配置文件
  //!  ---------------------------------------------------  Run  -------------------------------------------------------------------------- //
